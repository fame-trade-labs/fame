...
class Token {
  ...
  async mintTo(args: {
    ...
    console.log(`Mint ${args.amount} tokens to ${userATA.address.toBase58()}`);
  ...
  async getBalanceIntFor(user: Keypair | PublicKey) {
    ...
    return Math.floor(balance.value.uiAmount * 10 ** balance.value.decimals);
  }
...
describe("fame", () => {
  ...
  const createToken = async (args: {
    ...
    const accounts = {
      ...
      bondingCurve: anchor.web3.PublicKey.findProgramAddressSync(
        [Buffer.from("bonding_curve"), token1.mint.publicKey.toBuffer()],
        program.programId
      )[0],
      liquidityPool: anchor.web3.PublicKey.findProgramAddressSync(
        [Buffer.from("liquidity_pool"), token1.mint.publicKey.toBuffer()],
        program.programId
      )[0],
  ...
  const withdrawFees = async (args: { admin: Keypair; amount: anchor.BN }) => {
  ...
  it("Create a valid token", async () => {
    ...
    expect(event.token.toBase58()).to.equal(token1.mint.publicKey.toBase58());
